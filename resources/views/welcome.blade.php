            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <form action="{{ route('salary.calculate') }}" id="calculate_form" method="POST">
                    ...
                    <div id="error" style="display: none">
                        <b style="color: red">Ошибка валидации, проверьте данные и попробуйте еще раз</b>
                    </div><br>
                    ...
                    {{ csrf_field() }}
                    <div>
                        <label for="salary_tg">Оклад в тенге<br>
                            <input required style="width: 100%;" type="text" id="salary_tg" name="salary_tg" value="{{old('salary_tg') ?: ''}}" placeholder="Оклад в тенге">
                        </label>
                    </div>
                    <div>
                        <label for="norm_day">Норма дней в месяц (обычно 22)<br>
                            <input required style="width: 100%;" type="text" id="norm_day" name="norm_day" value="{{old('norm_day') ?: '22'}}" placeholder="22">
                        </label>
                    </div>
                    <div>
                        <label for="worked_day">Отработанное количество дней<br>
                            <input required style="width: 100%;" type="text" id="worked_day" name="worked_day" value="{{old('worked_day') ?: ''}}" placeholder="Отработанное количество дней">
                        </label>
                    </div>
                    <div>
                        <label for="tax_deduction">Имеется ли налоговый вычет 1 МЗП<br>
                            <select required style="width: 100%;" id="tax_deduction" name="tax_deduction">
                                <option selected disabled>Выберите параметр</option>
                                <option value="0">Не имеется</option>
                                <option value="1">Имеется</option>
                            </select>
                        </label>
                    </div>
                    <div>
                        <label for="year">Календарный год<br>
                    ...
                    <div>
                        <label for="months">Календарный месяц<br>
                    ...
                    <div>
                        <label for="pensioner">Является ли сотрудник пенсионером<br>
                            <select required style="width: 100%;" id="pensioner" name="pensioner">
                                <option selected disabled>Выберите параметр</option>
                                <option value="0">Не является</option>
                                <option value="1">Является</option>
                            </select>
                        </label>
                    </div>
                    <div>
                        <label for="disabled_person">Является ли сотрудник инвалидом, если да, то какой группы.<br>
                    ...
                    <br>
                    <div>
                        <a href="#" id="calculate_a" style="margin-right: 250px;">Показать результат</a>
                ...
                @if(isset($data))
                    <div id="success_post">
                        <span>Результат расчета с сохранением</span><br>
        ...
        $('#calculate_a').on('click', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $html = '<br><span>Результат расчета без сохранения</span><br>';
